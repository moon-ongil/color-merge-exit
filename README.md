...
2. `Assets/Scenes/Main.unity` → **Play** (문제 시 **Tools ▸ Color Exit ▸ Rebuild Main Scene**).
...
단일 시트 `Assets/Art/Textures/blocks_sheet.png`(투명 배경, 6열×6행: 컬러블록/출구포털/이동장애/고정벽/타일/UI)를 **`Tools ▸ Color Exit ▸ Set Up Art`** 로 처리:
...
배치: `Unity -batchmode -quit -executeMethod ColorExit.Editor.AssetSetup.Run`
